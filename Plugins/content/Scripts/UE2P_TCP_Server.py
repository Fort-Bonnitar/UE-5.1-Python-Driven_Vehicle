...
def load_models():
    global model_steering
    global model_throttle
...
def use_models(data_string):
    ...
    input_data = [throttle, left, right, speed]
    X_new = np.array(input_data).reshape(1, -1)

    # Create a DataFrame with feature names
    X_newest = pd.DataFrame(X_new, columns=['throttle', 'left', 'right', 'speed'])

    throttle = model_throttle.predict(X_newest)
    steering = model_steering.predict(X_newest)
...
##############################################################################################################
        ...
        # if run_type == 'load':
        load_models()
        steering_out, throttle_out = use_models(data_string)
